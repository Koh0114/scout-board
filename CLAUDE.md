...
1. 수비 지표 추가 (태클·인터셉트) — 수비수 스카우팅이 열린다. 데이터 소스부터 찾아야 함
2. 선수 나이·이적료 — 영입 현실성 판단. Transfermarkt 계열
3. 선수 비교 뷰 — 두 명 겹쳐 보는 레이더/막대
4. 팀 간 직접 비교 (지금은 한 팀씩만 봄)
5. 모바일 레이아웃 손보기 — 820px 아래에서 테이블이 좁다
